  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => onNavigate('engagement')} className="btn-primary">⚡ エンゲージメント実行へ</button>
      </div>
      ...
      <form onSubmit={handleAdd} className="card p-5 space-y-3">
        ...
        <div className="flex gap-3">
          <div className="flex-1">
            <input className="input" type="url" placeholder="https://x.com/username/status/1234567890"
              value={url} onChange={e => setUrl(e.target.value)} required />
          </div>
          <div className="w-64">
            <input className="input" type="text" placeholder="メモ（任意）"
        ...
        <div className="space-y-3">
          {posts.map(post => (
            <div key={post.id} className="card overflow-hidden">
              <div className="p-4 cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => handleSelectPost(post.id)}>
                <div className="flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      {post.author && <span className="font-medium text-gray-700 text-sm">@{post.author}</span>}
                    ...
                    <a href={post.url} target="_blank" rel="noreferrer"
                      ...
                      onClick={e => e.stopPropagation()}>{post.url}</a>
                    {post.description && <p className="text-gray-500 text-xs mt-1">{post.description}</p>}
                  </div>
                  <div className="flex gap-3 shrink-0">
                    <div className="text-center"><p className="text-lg font-bold text-pink-600">{post.like_done}</p><p className="text-xs text-gray-400">いいね</p></div>
                    <div className="text-center"><p className="text-lg font-bold text-green-600">{post.repost_done}</p><p className="text-xs text-gray-400">RT</p></div>
                    <div className="text-center"><p className="text-lg font-bold text-blue-600">{post.reply_done}</p><p className="text-xs text-gray-400">返信</p></div>
                    {post.failed_count > 0 && (
                      <div className="text-center"><p className="text-lg font-bold text-red-500">{post.failed_count}</p><p className="text-xs text-gray-400">失敗</p></div>
                    )}
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-gray-400 text-xs">{new Date(post.created_at).toLocaleDateString('ja-JP')}</span>
                    <span className="text-gray-300">{selectedPost === post.id ? '▲' : '▼'}</span>
                    <button onClick={e => { e.stopPropagation(); handleDelete(post.id) }} className="btn-danger">削除</button>
                  </div>
                </div>
              </div>
              ...
              {selectedPost === post.id && (
                <div className="border-t border-gray-100 bg-gray-50 p-4">
                      ...
                      <table className="text-sm w-full">
                        <thead>
                          <tr className="text-gray-500 text-xs border-b border-gray-200">
                            ...
                            {ACTION_TYPES.map(t => <th key={t} className="text-center pb-2 px-3">{t}</th>)}
                          </tr>
                        </thead>
                        <tbody>
                          {accountStatuses.map(acc => (
                            <tr key={acc.id} className="border-b border-gray-100 last:border-0">
                              ...
                              {ACTION_TYPES.map(t => (
                                <td key={t} className="py-2 px-3 text-center">{statusBadge(acc.actions[t]?.status)}</td>
                              ))}
                            </tr>
                          ))}
                        </tbody>
                  ...
                  <div className="mt-3">
                    <button onClick={() => onNavigate('engagement')} className="btn-primary text-xs py-1.5">
                      ⚡ このポストでエンゲージメントを実行
                    </button>
                  </div>
                </div>
