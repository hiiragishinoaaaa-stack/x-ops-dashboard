      ...
      <div className="grid grid-cols-5 gap-6">
        <div className="col-span-3 space-y-4">
          <div className="card p-5 space-y-3">
            ...
            {currentTarget && (
              <a href={currentTarget.url} target="_blank" rel="noreferrer"
                className="text-blue-500 text-xs hover:underline truncate block">{currentTarget.url}</a>
            )}
            {currentTarget && (
          ...
          <div className="card p-5 space-y-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex items-center gap-1">
                <input type="number" className="w-16 border border-gray-300 rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500"
                  placeholder="N" min={1} max={accounts.length} value={countInput}
              ...
              {accounts.map((acc, i) => (
                <label key={acc.id}
                  ...
                  <input type="checkbox" checked={selectedAccounts.has(acc.id)}
                    onChange={() => toggleAccount(acc.id)} className="rounded text-blue-600" />
                  ...
                  {result.results.map(r => (
                    <div key={r.account_id}
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm ${
                        ...
                        r.skipped ? 'bg-gray-50 text-gray-500' : 'bg-red-50 text-red-700'
                      }`}>
                      ...
                      {r.skipped && <span className="text-xs ml-auto text-gray-400">実行済み</span>}
                    </div>
                  ))}
                </div>
                <button onClick={() => setResult(null)} className="btn-secondary w-full">クリア</button>
              </div>
            )}
          </div>
        </div>
